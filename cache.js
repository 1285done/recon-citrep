...
module.exports = function (setup) {
	...
	module.query = function (id, cb) {
		...
		esi.names(id).then(function (item) {
			...
			log.error("cache.query: Error for esi.names", {
				err,
				id
			});
		});
	...
	module.addToDb = function (data, expiresIn, cb) {
		...
		db.update(doc, data, {
			...
			if (err) {
				log.error("addToDb: Error for db.update", {
					err,
					id: data.id
				});
	...
	module.get = function (id, expiresIn, cb) {
		db.findOne({
			...
			if (err) log.error("get: Error for db.findOne", {
				err,
				id
			});
			if (doc === null) {
	...
	module.massQuery = function (ids, cb) {
		...
		db.find({
			...
			if (err) log.error("massQuery: Error for db.find", {
				err,
				ids
			});
			var fullquery = [];
			...
			esi.names(newBulkSearch).then(function (items) {
				db.insertMany(items, function (err, result) {
					if (err) log.error("cache.massQuery: Error for db.insertMany", {
						err,
						items
					});
