...
module.exports = function (setup) {
    ...
    module.get = function (cb) {
        db.find({
            ...
            if (err) log.error("whitelist.get: Error for db.find", {
                err
            });
            cb(docs);
        })
        ...
        module.isAllowed(null, data.id, data.id, function (onWhitelist) {
            if (!onWhitelist) {
                db.insert(data, function (err) {
                    if (err) log.error("whitelist.store: Error for db.insert", {
                        err,
                        data
                    });
                    cb(null);
